...
# 真空の透磁率 4`π 1e-7, 物理単位
#`newton `ampere^-2 == henry/meter == weber/(ampere meter)
mu0 = 1.2566370614e-6
# 真空の誘電率:upsilon0: ε0 == 1/(`c^2 4`π 1e-7)==Q**2 / (newton * M ** 2) == coulomb/(volt meter)
ep0 = 8.854187816e-12


#print "At kNumeric before import sfFnctns as sf"
#import pdb; pdb.set_trace()
import sfFnctns as sf
import numpy as sc
np = sc
#print "At kNumeric after import sfFnctns as sf"

"""'
# abs(..) は built in 関数。これを下の absF(..) の動作に変更するのは危険すぎる。
absF = sf.ClAF(lambda x: abs(x)) # abs(..) must be called in __abs__(..)__.
'"""
def __absF(ag):
...
exp, sin =sf.ClAF(sc.exp), sf.sf.ClAF(sc.sin)
cos, tan, sinh = sf.ClAF(sc.cos),sf.ClAF(sc.tan),sf.ClAF(sc.sinh)
cosh, tanh, = sf.ClAF(sc.cosh),sf.ClAF(sc.tanh)
arctan = sf.ClAF(sc.arctan)
...
#log10 = sf.ClAF(sc.log10)
def __log10(ag):
    if hasattr(ag,'__len__'):
        import scipy
        if isinstance(ag,sc.ndarray) and not isinstance(ag, sf.ClTensor):
            return scipy.log10(ag)
        else:
            return sf.krry(scipy.log10(ag))
    elif (isinstance(ag, (int, float)) and ag < 0):
        return sc.log10(complex(ag))
    else:
        return sc.log10(ag)
...
def __sqrt(ag):
    if hasattr(ag, 'is_Atom'):
        ...
        return sc.sqrt(complex(ag))
    elif ( isinstance(ag, sc.ndarray) ):
...
def __arcsin(ag):
    if ag%1==0 and not(ag in [-1,0,1]):
        return sc.arcsin(complex(ag))
    else:
        return sc.arcsin(ag)

def __arccos(ag):
    if ag%1==0 and not(ag in [-1,0,1]):
        return sc.arccos(complex(ag))
    else:
        return sc.arccos(ag)
...
def ifft(sqAg, n=None, axis = -1):
    ...
    '"""
    import numpy.fft as fp
    return sf.krry(fp.ifft(sqAg, n, axis))
...
def nft(sqAg, n=None, axis = -1):
    ...
    import numpy.fft as fp
    #return sf.krry(fp.fft(sqAg, n, axis)/sqrt(2))
    return sf.krry(fp.fft(sqAg, n, axis))/sqrt(len(sqAg))
...
def inft(sqAg, n=None, axis = -1):
    ...
    #import scipy.fftpack as fp
    import numpy.fft as fp
    #return sf.krry(sqrt(2)*fp.ifft(sqAg, n, axis))
    return sf.krry(fp.ifft(sqAg, n, axis))*sqrt(len(sqAg))

diag = lambda x,k=0:sf.krry(sc.diag(x,k))
...
def eig(a, b=None, left=False, right=True, overwrite_a=False
    ...
    return (sf.krry(tplAt[0]), sf.krry(tplAt[1]))

def eigvals(a):
    import numpy.linalg as sl
    return sf.krry(sl.eigvals(a))
...
def eigvalsh(a, lower='L'):
    import numpy.linalg as sl
    return sf.krry(sl.eigvalsh(a, lower))

def expm(mtAg):
    """' return krry matrix exponential value
    '"""
    #import numpy.linalg as sl  # there is no expm in numpy.linalg
    import scipy.linalg as sl
    return sf.krry(sl.expm(mtAg))

def logm(mtAg):
    """' return krry matrix logarithm value
    '"""
    #import numpy.linalg as sl  # there is no logm in numpy.linalg
    import scipy.linalg as sl
    return sf.krry(sl.logm(mtAg))

def sqrtm(mtAg):
    """' return krry matrix square root value
    '"""
    # 11y06m22d append
    import scipy.linalg as sl
    return sf.krry(sl.sqrtm(mtAg))
...
class Nl_(object):  # Nabla
    ...
    def __init__(self, fnAg, dim = None):
        ...
        self.m_arP_ = sc.array([P_(k,fnAg) for k in range(dim)])
        self.shape = self.m_arP_.shape
        self.m_inDim = dim

    def __call__(self, *sqAg):
        return sf.krry([fnAt(*sqAg) for fnAt in self.m_arP_])
...
class Dv_(object):  # Divergence
    ...
    def __init__(self, *sqFnAg):
        if isinstance(sqFnAg[0], Nl_):
            ...
            or isinstance( sqFnAg[0], type( sc.array([1.0, 2]) ) )  ):
            sqFnAg = sqFnAg[0]

        self.m_arP_ = sc.array([P_(k, sqFnAg[k]) for k in range(len(sqFnAg))])
...
class Jc_(object):
    ...
    def __call__(self, *sqAg):
        ...
        if isinstance(elmAt, sf.ClTensor):
            ...
            return sf.krry(dctDctAt)
        else:
            return sf.krry(dctAt)
...
def __norm(*sqAg):
    ...
    if isinstance( sqAg, (sf.ClTensor, sf.ClFldTns) ):
        sqAg = sqAg.r
    elif isinstance( sqAg, sc.ndarray) and sqAg.shape==():
        sqAg = sc.ravel(sqAg)
        #return sl.norm(sqAg)
...
# ClAfNorm により 1/r≡ 1/norm(~[`X,`Y,`Z])(...) を記述可能にする
    ...
    for i in range(len(sqXN)-1):
        def elmntF(x, n, arA, arB):
            ...
            if isinstance(x[0], complex):
                rtnAr=sc.zeros(lenAt, complex)
            else:
                rtnAr=sc.zeros(lenAt, float)
...
def cplx2real(vct):
    ...
    if isinstance(vct, int) or isinstance(vct, float):
        return sf.krry(vct, 0)
    elif vct is complex:
        return sf.krry(vct.real, vct.imag)
    else:
        vctAt = sc.zeros(2*len(vct))
...
def real2cplx(vct):
    ...
    vctAt = sc.zeros(len(vct)/2, complex)
...
def cOdeInt(fncAg, y0, t, args=() ):
    ...
    cplxRtAt = sc.zeros([len(rtAt), len(rtAt[0])/2], complex)
...
def solvePDE(dctBoundary, fncRecurring, inIteration=64, dfltValue = 0
    ...
    elif isinstance(dctBoundary, sc.ndarray):
...
def dct2(sqAg):
    """' Type2 Discrete Cosine Transformation for sequence argment
        return krry vector with same norm size for argment sequence.
    e.g.
    ...
    if isinstance(sqAg, list) and isinstance(sqAg[0],int):
        sqAg[0] = float(sqAg[0])
    elif isinstance(sqAg, sc.ndarray) and (
            sqAg.dtype in (sc.int32,sc.int16,sc.int64)):
        sqAg = sc.array(sqAg,dtype=float)

    import scipy.fftpack as md
    return sf.krry(md.dct(sqAg, type=2, norm='ortho'))
...
def idct2(sqAg):
    ...
    import scipy.fftpack as md
    return sf.krry(md.dct(sqAg, type=3, norm='ortho'))
...
def quadC(*sqAg, **dctAg):
    ...
    f=0.1;quadC(λ x   :exp(-x^2) exp(2 pi `i x f), -sc.inf, sc.inf)
    ===============================
    (1.60587519197+0j)

    Fourier transformed function with a parameter f
    (λ f:(quadC( λ t,f:exp(-t^2) exp(2 pi `i f t), -sc.inf, sc.inf,args=(f,))))(0.1)
    ===============================
    (1.60587519197+0j)
...
class ClODE(object):
    def __init__(self, f, x0, t0=0):
        ...
        self.m_ode.set_initial_value(sf.krry(x0))
        self.m_x0 = x0
        self.m_lenX0 = len(x0)
        self.m_t0=None
    ...
    def __call__(self, t, N=50, t0=None):
        ...
        self.m_t0 += t
        if self.m_lenX0 == 1:
            return sf.krry(lstRtn)[:,0]
        else:
            return sf.krry(lstRtn)
...
def odeint(f, x0, N=50., t0=0):
    ...
    elif isinstance(f, sf.ClFldTns) and issubclass(f.m_type, sf.ClAF):
...
def randint(*sqAg, **dctAg):
    if len(dctAg)==0:
        ...
        return sf.krry(sf.sc.random.randint(*sqAg, **dctAg), dtype=int)
...
def randn(*sqAg):
    if len(sqAg) == 0:
        ...
        return sf.krry(sf.sc.random.randn(*sqAg))
...
# SNf:00062 invalid: 0.97B?: add cnvlv function date:2014/10/04
#   We have judged that below codes are not important. If they would be needed
# then them implement in a sfCrrintIni.py for appliction directories.
#def continueHolomorphically(sq):
#    if hasattr(sq, '__getitem__'):
#        if isinstance(sq, np.ndarray):
#            pass
#        else:
#            sq=sf.krry__(sq)
#    elif hasattr(sq, '__iter__'):
#        sq=sf.krry__(tuple(sq))
#    else:
#        assert False, "In continueHolomorphically(.), you set a wrong parameter: "+str(sq)
#
#    vctZ=sq
#    N=len(vctZ)
#    arZ = sf.kzrs([N,N], complex)
#
#    vctZd = sf.kzrs(N, complex)
#    vctZd[:]=sq[:]
#
#    # set diagonal data on real axis
#    arZ.r[0:N*N:N+1] = vctZ[0:N]
#
#    # set matrix data on image axis > 0 side
#    for i in range(1,N):
#        # コーシー・リーマンの方程式
#        # ∂x(u) = ∂y(v) ==> ∂x(u) = ∂y(`i v)/`i
#        # ∂x(v) =-∂y(u) ==> ∂x(`i v) =∂y(u) /`i  ==> ∂y(u+`i v)== `i ∂x(u+`i v)
#        # `i V(:,:+0.5Δ)=`i V(i,:) + 0.5∂x(U) `i
#        # U(:,:+0.5Δ)=U(i,:) - 0.5∂x(`i V)/`i
#        # (U+V `i)(:+0.5Δ,:+0.5Δ)==0.5*(vctZ+shftSq(vctZ,-1))
#        # 0.5∂y(U+`i V) == `i ∂x(u+`i v)== 0.5*(vctZ - shftSq(vctZ,-1))`i
#        vctZ = 0.5*(vctZ+sf.shftSq(vctZ,-1)) + 0.5*(vctZ - sf.shftSq(vctZ,-1))*1j
#                                                                 
#        arZ.r[i:N*(N-i):N+1] = vctZ[:N-i]
#
#    # set matrix data on image axis < 0 side
#    vctZ = vctZd
#    for i in range(1,N):
#        #vctAt = vctZ - sf.shftSq(vctZ,-1)                           
#        # ∂x(U),∂x(V) # V(:,:+0.5Δ)=V(i,:) - 0.5∂x(U)*1j # U(:,:+0.5Δ)=V(i,:) + 0.5∂x(Y)*1j
#        vctZ = 0.5*(vctZ+sf.shftSq(vctZ,-1)) - 0.5*(vctZ - sf.shftSq(vctZ,-1))*1j         
#        arZ.r[i*N:N*N:N+1] = vctZ[:N-i]
#
#    return arZ
#
#cntnHl = continueHolomorphically
#
#def continueHolomorphicallyWithFourierTr(sq):
#    """' Contniue sequence parameter holomorphically with Fourier transform.
#
#    ar = fft(sq)/sqrt(N)    # Fourier transform with serving norm
#
#    ar[k] --> matrix:
#              exp(`i 2pi k t/N) on t∈[0,1] klsp(1/2+k+`i k/2,1/2+1-k+`i k/2,N-k)
#                                        and klsp(1/2+k-`i k/2,1/2+1-k-`i k/2,N-k)
#    '"""
#    if hasattr(sq, '__getitem__'):
#        pass
#    elif hasattr(sq, '__iter__'):
#        sq=tuple(sq)
#    else:
#        assert False, "In continueHolomorphicallyWithFourierTr(.), you set a wrong parameter: "+str(sq)
#
#    N=len(sq)
#    mtZ = np.zeros([N,N], complex)
#
#    arF = np.fft.fft(sq)/np.sqrt(N)
#    if N%2==0:
#        arFpls=arF[:N//2]
#        arFmns=arF[N//2:][::-1]
#    else:
...
#        arFmns=arF[N//2+1:][::-1]
#
#    # set ar[0] cost data on matrix
#    #mtZ.ravel()[:] = [arF[0]]*(N*N)
#
#    # set matrix data on image axis both sides
...
#                # plus frequency side
#                if m<N//2:
#                    mtZ.ravel()[0:N*N:N+1] += [arFpls[m] * np.exp(1j*2*np.pi*m*t) for t
#                                       in np.linspace(0, 1, N+1)[:-1]]
#
#                # minus frequecy side
#                if m>0 and (m<N//2 or N%2==0):
#                    mtZ.ravel()[0:N*N:N+1] += [arFmns[m-1] * np.exp(-1j*2*np.pi*m*t) for t
#                                       in np.linspace(0, 1, N+1)[:-1]]
#            else:
#                # plus frequency side
#                if m<N//2:
#                    mtZ.ravel()[k:N*(N-k):N+1] += [arFpls[m] * np.exp(1j*2*np.pi*m*t) for t
#                                       in np.linspace((k/2.0+1j*k/2.0)/N, 1-(k/2.0)/N+(1j*k/2.0)/N, N-k+1)[:-1]]
#
#                    mtZ.ravel()[k*N:N*N:N+1]   += [arFpls[m] * np.exp(1j*2*np.pi*m*t) for t
#                                       in np.linspace((k/2.0-1j*k/2.0)/N, 1-(k/2.0)/N-(1j*k/2.0)/N, N-k+1)[:-1]]
#
#                # minus frequecy side
#                if m>0 and (m<N//2 or N%2==0):
#                    mtZ.ravel()[k:N*(N-k):N+1] += [arFmns[m-1] * np.exp(-1j*2*np.pi*m*t) for t
#                                       in np.linspace((k/2.0+1j*k/2.0)/N, 1-(k/2.0)/N+(1j*k/2.0)/N, N-k+1)[:-1]]
#
#                    mtZ.ravel()[k*N:N*N:N+1]   += [arFmns[m-1] * np.exp(-1j*2*np.pi*m*t) for t
#                                       in np.linspace((k/2.0-1j*k/2.0)/N, 1-(k/2.0)/N-(1j*k/2.0)/N, N-k+1)[:-1]]
#
#    return sf.krry(mtZ)
#
