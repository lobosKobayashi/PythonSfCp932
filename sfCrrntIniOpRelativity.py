...
def tt():
    ...
    Ahk_bq_= Cnk_bq_ /snk_bq_ / sf.sqrt(e0nk_bq_) 
    #k_Ah_bq____ = k_Cn_bq____ / k_sn_bq____ / sf.sqrt(k_e0n_bq____)                # wrong
    #k_Ah_bq____ = k_Cn_bq____ /(k_cn_bq____ * k_sn_bq____) / sf.sqrt(k_e0n_bq____) # correct too
    # rot H = j date:2015/05/15 (金) time:09:03
    #k_Ah_bq____ = k_Cn_bq____ / k_sn_bq____ * sf.sqrt(k_u0n_bq____)                 # correct
    Chk_bq_ = Cnk_bq_ / sf.sqrt(e0nk_bq_)
    Vhk_bq_ = Vnk_bq_ * sf.sqrt(e0nk_bq_)
    Thk_bq_ = Tnk_bq_ / sf.sqrt(u0nk_bq_)

    pQhk_bq_= pQnk_bq_/ sf.sqrt(e0nk_bq_)
    eQhk_bq_=-pQhk_bq_
    ...
    # rot H = 1/c j date:2015/05/15 (金) time:09:03
    Alk_bq_= ut.A / sf.sqrt(ut.e0)/336066.81876646896  # date:2015/05/17 (日) time:08:14
    Clk_bq_= ut.C / sf.sqrt(ut.e0)/336066.81876646896
    Vlk_bq_= ut.V * sf.sqrt(ut.e0)/2.9755987326285099e-6
    Tlk_bq_= ut.T / sf.sqrt(ut.u0)/892.06205807638571

    pQlk_bq_= 1.6021892e-19 * ut.C / sf.sqrt(ut.e0)
    eQlk_bq_=-pQlk_bq_
    ...
    pQhkk_bq_=Ckk_bq_/ sf.sqrt(e0kk_bq_)
    eQhkk_bq_= -pQhkk_bq_

    Chkk_bq_=Ckk_bq_/ sf.sqrt(e0kk_bq_)/0.302824523103951
    # rot H = 1/c j date:2015/05/15 (金) time:09:03
    Ahkk_bq_=Ckk_bq_/ skk_bq_/ sf.sqrt(e0kk_bq_)/0.302824523103951
    Vhkk_bq_=Vkk_bq_* sf.sqrt(e0kk_bq_)/3.30224246619792
    Thkk_bq_=Tkk_bq_/ sf.sqrt(u0kk_bq_)/3.30224246619792
...
def mnl(v):
    ...
    if not isinstance(v, sf.np.ndarray):
        v = sf.kryO(v)
    #assert v * v < 0, "You set a space like vector:"+str(v)
    assert sf.np.dot(v, v) < 0, "You set a space like vector:"+str(v)
    #return 1j*v/sf.sqrt(v*v)
    return 1j*v/sf.sqrt(sf.np.dot(v,v))
