...
class kfs(frozenset): # same as kfs in ptGrp.py
    """' frozenset that has
    ...
    def __getattr__(self, name):
        if name == "oi" or name == "orderedIterator":
            ...
            if "p" in self.__dict__:
                ...
                tplAt = tuple(sorted(tuple(self), key=lambda x: tuple(x) if isinstance(x, kfs) else x))
...
class ClZp(object):
    ...
    def __add__(self, ag):
        if isinstance(ag, (             oc.ClOctonion)):
...
if __name__ == "__main__":
    ...
    assert Sb(0,3,1,2) == [0,3,1,2]
    assert Sb(0,3,1,2) == sf.kryO([0,3,1,2])
