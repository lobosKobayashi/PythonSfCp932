...
x + 0.6667 x + 13.67 x + 7.333 x + 69.33 x + 26 x + 155 x + 30 x + 129
    ...
    def __elmntNmDn(self):
        """' eliminate 0 term that exists both of the nemerater and denominator
        '"""
    ...
    def __call__(self, val):
        if isinstance(val, (tuple, list, sc.ndarray)):
            return sc.array([self(x) for x in val])
        else:
    ...
    def __neg__(self):
        clRtn = ClRtnl(self)
        clRtn.m_plNumer = poly1d(-sc.array(clRtn.m_plNumer.coeffs))
        return clRtn
    ...
    def getAnImpls(self, time, dataN = 128):
        ...
        arrX = si.odeint( func, sf.kryO( [0]*(N-1)+[1] )
                , sf.arSqnc(0,dataN, float(time)/dataN) )

        #print arrX
        #print arrX[3,:]
        # 12.12.13 return sc.dot(arrX, C)
        return sf.kryO(sc.dot(arrX, C).view())
    ...
    def getAnRspns(self, time, input = sf.arsq(1, 128, 0.0) ):
        ...
        if ( len(numeratorAt) == N+1):
            ...
            numeratorAt = list(
                    ...
                    sf.kryO(numeratorAt[0:-1]) + sc.dot(D,sf.kryO(denominatorAt) ))
        ...
        for inpAt in input:
            def func(x,t):
                return sc.dot(A,x) + sc.dot(inpAt,B)

            lsX.append( si.odeint(func, lsX[-1], [0,float(time)/dataN])[-1] )

        return sc.dot(lsX[:-1], C)+sc.dot(D,sf.kryO(input))
    ...
    def getDgImpls(self, dataN = 128):
        ...
        lenAt = len(numeratorAt)
        C = numeratorAt + [0]*(N-lenAt) # numeratorAt is list
        B = sc.zeros(N)
        B[-1] = 1
    ...
    def plotAnRspns(self, time, input = sf.arsq(1, 128, 0.0), color=(0,1,1) ):
        ...
        vsVal = sc.dot(lsX[:-1], C)+sc.dot(D,sf.kryO(input))
        #import pdb; pdb.set_trace()
        def f(x):
            inAt = int(R*x)
            return vsVal[inAt]
